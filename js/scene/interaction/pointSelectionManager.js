...
export function setupPointSelection({ renderer, camera, model, visor }) {
  ...
  function activarRaycast() {
    function handler(e) {
      ...
      if (!hits.length) {
        console.log(`[RAYCAST][Visor${visor}] No se ha tocado ningún punto.`);
        return;
      }
      ...
      console.log(`[RAYCAST][Visor${visor}] Punto seleccionado:`, {
      ...
      console.log(`[RAYCAST][Visor${visor}] Punto registrado y handler desactivado`);
    }
    canvas.removeEventListener('click', handler);
    canvas.addEventListener('click', handler);
    console.log(`[RAYCAST][Visor${visor}] Handler de raycast activado para selección`);
  }
...
window.addEventListener('activarSeleccionPunto', (e) => {
  ...
  if (activarRaycastPorVisor[visor]) {
    ...
    console.log(`[SELECCION][Visor${visor}] Raycast activado para index ${index}`);
  } else {
    console.warn(`[SELECCION][Visor${visor}] No existe activarRaycast para este visor`);
  }
});
